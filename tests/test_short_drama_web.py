...
class ShortDramaWebTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_dashboard_pages_and_show_api(self):
        ...
        page_source = response.text + static_js.text
        self.assertIn("startEditAccount", static_js.text)
        self.assertIn("batchIgnoreReviews", static_js.text)
        self.assertIn("startHistoryBackfill", static_js.text)
        self.assertIn("historyProgress", static_js.text)
        self.assertIn("history-progress-track", page_source)
        self.assertIn('status === "idle"', page_source)
        self.assertIn("尚未开始历史扫描", static_js.text)
        self.assertIn("抖音未提供历史总页数", static_js.text)
        self.assertIn("reparseAccount", static_js.text)
        self.assertIn("reparseVideo", static_js.text)
        self.assertIn("videoDescription", static_js.text)
        self.assertIn("parserEvidence", static_js.text)
