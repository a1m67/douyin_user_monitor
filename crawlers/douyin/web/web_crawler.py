...
from crawlers.douyin.web.models import (
    ...
    UserLive2, UserMix, UserPost, UserSearch, GeneralSearch
)
# 抖音应用的工具类
...
# 配置文件路径（可通过 DYMON_CRAWLER_CONFIG / set_config_path 注入）
path = os.path.abspath(os.path.dirname(__file__))

from crawlers.douyin.web.config_loader import get_config, set_config_path  # noqa: E402
from crawlers.douyin.web.utils import _reload_token_manager_from_config  # noqa: E402
...
class DouyinWebCrawler:
    ...
    # 搜索用户（通过抖音号/昵称搜索）
    async def fetch_user_search(self, keyword: str, offset: int = 0, count: int = 10):
        """
        通过关键词搜索用户
        Args:
            keyword: 搜索关键词（抖音号或昵称）
            offset: 偏移量，用于分页
            count: 返回数量
        Returns:
            搜索结果，包含用户列表
        ...
        base_crawler = BaseCrawler(proxies=kwargs["proxies"], crawler_headers=kwargs["headers"])
        async with base_crawler as crawler:
            params = UserSearch(keyword=keyword, offset=offset, count=count)
            # 生成带有a_bogus加密参数的Endpoint
            params_dict = params.dict()
            params_dict["msToken"] = ''
            a_bogus = BogusManager.ab_model_2_endpoint(params_dict, kwargs["headers"]["User-Agent"])
            endpoint = f"{DouyinAPIEndpoints.USER_SEARCH}?{urlencode(params_dict)}&a_bogus={a_bogus}"
            response = await crawler.fetch_get_json(endpoint)
        return response

    # 综合搜索（通过关键词搜索，包含用户、视频等）
    async def fetch_general_search(self, keyword: str, offset: int = 0, count: int = 10):
        """
        综合搜索
        Args:
            keyword: 搜索关键词
        ...
        base_crawler = BaseCrawler(proxies=kwargs["proxies"], crawler_headers=kwargs["headers"])
        async with base_crawler as crawler:
            params = GeneralSearch(keyword=keyword, offset=offset, count=count)
            # 生成带有a_bogus加密参数的Endpoint
            params_dict = params.dict()
            params_dict["msToken"] = ''
            a_bogus = BogusManager.ab_model_2_endpoint(params_dict, kwargs["headers"]["User-Agent"])
    ...
    async def update_cookie(self, cookie: str):
        ...
        Args:
            cookie: 新的Cookie值
        """
        from crawlers.douyin.web import config_loader as _cfg_loader
        ...
        _cfg_loader.load_config(force_reload=True)
    ...
    async def main(self):
        ...
        # 获取用户喜欢作品数据
        # sec_user_id = "EXAMPLE_SEC_USER_ID"
        # max_cursor = 0
        # count = 10
