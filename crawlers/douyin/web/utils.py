...
from crawlers.douyin.web.config_loader import get_config, load_config, set_config_path  # noqa: E402


def _reload_token_manager_from_config() -> None:
    """Refresh TokenManager class fields from current crawler config."""
    cfg = get_config()
    douyin_manager = cfg.get("TokenManager", {}).get("douyin") or {}
    proxies_conf = douyin_manager.get("proxies") or {}
    TokenManager.douyin_manager = douyin_manager
    TokenManager.token_conf = douyin_manager.get("msToken", None)
    TokenManager.ttwid_conf = douyin_manager.get("ttwid", None)
    TokenManager.proxies_conf = proxies_conf
...
try:
    ...
    # Config path will be set by InProcessDouyinClient before use.
    pass
